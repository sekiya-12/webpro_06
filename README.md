# webpro_06
## app5.jsのプログラムについて
### 24G1084 関屋　咲希　（２０２４年１１月１８日）

## はじめに
　app5.jsのプログラムについて説明する．まずapp5.jsにNode.jsとExpressを使ったサーバーの基本的な設定を行った．次に以下のプログラムを実装した．
- ```じゃんけんシステム```：ユーザーがじゃんけんの手を決めてコンピュータとの対戦結果を基に勝敗，勝数，試合数を表示するシステム．
- ```ガチャシステム```：ユーザーが「ガチャを引く」のボタンを押し，その結果がランダムに決定され，表示されるシステム．
- ```おみくじシステム```：ユーザーが「占う」のボタンを押し，その結果が1〜６の数字でランダムに決定され，その数字に対応した運勢が表示されるシステム．
- ```クイズシステム```：Expressフレームワークを使用してユーザーがクイズに回答した後，その結果が正解または不正解であるか判定して表示するシステム．
...
これによりapp5.jsが起動し，使用したいプログラムを利用することができる．
## じゃんけんの仕様手順
...
## じゃんけんの仕様
　このじゃんけんシステムではユーザーがじゃんけんの手を決めてコンピュータとの対戦結果を基に勝敗，勝数，試合数を表示する．ユーザーの手とコンピュータの手を比較し，結果に応じて勝ち数や総試合数を更新する．結果はテンプレートを使って画面に表示される．
...
## ガチャの仕様手順
1. ターミナルを開き```node app5.js```でプログラムを起動する．
1. Webブラウザで```localhost:8080/public/gacha.html```にアクセスする．
1. 画面に表示されている**ガチャを引く**のボタンを押す．
1. ユーザーにSSR，SR，Rのいずれかの結果を表示する．
## ガチャの仕様
　このガチャシステムではユーザーが「ガチャを引く」のボタンを押し，その結果がランダムに決定され，表示される．getリクエストでガチャフォームを表示しpostリクエストで1〜3の間のランダムな数字が生成され，ガチャの結果（SSR，SR，R）を決定する．結果はテンプレートを使って画面に表示される．
...
1. ユーザーに大吉，中吉，小吉，末吉，凶，大凶のいずれかの結果を表示する．

## おみくじの仕様
　このおみくじシステムはユーザーが「占う」のボタンを押し，その結果がランダムに決定され，表示される．1〜6の間のランダムな数字が生成され，それに対応する運勢（大吉，中吉，小吉，末吉，凶，大凶）が決定する．結果はテンプレートを使って画面に表示される．
...
## クイズの仕様手順
1. ターミナルを開き```node app5.js```でプログラムを起動する．
1. Webブラウザで```localhost:8080/public/quiz.html```にアクセスする．
1. 画面に表示されている問題の答えを1〜3から選択し，入力する．
1. ユーザーに正解，不正解のどちらかを表示する．

## クイズの仕様
　このクイズシステムはNode.jsのExpressフレームワークを使用してユーザーがクイズに回答した後，その結果が正解または不正解であるか判定して表示する．結果はテンプレートを使って画面に表示される．

　まず```app.get("/quiz", (req, res) ```で/quizというURLに対するリクエストを処理する．```quiz.html```で問題や選択肢などが表示され，ユーザーはその問題に対する答えをフォームで入力する．
ユーザーがクイズでリクエストを送信すると，フォームがpostリクエストとして送信され/quiz-resultにデータが届く．このときサーバー側では ```req.body.answer```を受け取って処理する．
...
## GitHubでの管理方法
　GitHubで管理する際，誰が変更したか明らかにするためユーザ名とメールアドレスを登録する．
1. $ git config --global user.name "ユーザー名"
1. $ git config --global user.email "メールアドレス"

この作業を1回だけ実行しておく．

　修正したファイルや新しく追加したファイルをGitHubにアップロードし管理する際，以下の手順で行う．
1. ターミナルを開く．
1. 以下のコマンドを実行する．
- ＄　git add.
